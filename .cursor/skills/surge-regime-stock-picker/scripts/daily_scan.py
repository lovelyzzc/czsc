...
REVIEW_LIMIT = max(1, int(os.getenv("SURGE_REVIEW_LIMIT", "5")))  # 仅限制人工复盘表，不截断完整候选日志
...
def _report_default(results, name_map, industry_map, metadata_available):
    ...
    new_actionable = [r for r in actionable if r["新鲜度"] == 0]
    tracking = [r for r in actionable if r["新鲜度"] > 0]
    review_rows = sr.select_diverse_rows(
        new_actionable,
        REVIEW_LIMIT,
        priority_key="优先级",
    ...
    review_rank = {r["代码"]: i for i, r in enumerate(review_rows, 1)}
    for r in results:
        r["今日新增"] = r["新鲜度"] == 0
        r["复盘精选"] = r["代码"] in review_rank
        r["复盘序号"] = review_rank.get(r["代码"])
    n_a = sum(1 for r in actionable if r["等级"] == "A")

    print(f"\n{'=' * 150}")
    print(
        f"  {scan_date} 主升浪结构扫描 | 今日新增可操作 {len(new_actionable)} 只 / "
        f"人工复盘 {len(review_rows)} 只（上限 {REVIEW_LIMIT}）| 存量跟踪 {len(tracking)} 只"
    )
    print(
        f"  完整结构池 {len(results)} 只：通过硬过滤 {len(actionable)}（A级 {n_a}）/ "
        f"观察池 {len(watchlist)}；完整数据全部留档，不因复盘上限删样本"
    )
    ...
    print("  ⚠ 复盘名单用于结构与执行核验；priority 仅作稳定展示排序，历史审计未证明其有收益预测力")
    print(f"{'=' * 150}")
    header = (
    ...
    if not review_rows:
        print("\n  今日没有“新鲜度=0 且通过硬过滤”的新信号；存量候选仅继续自动跟踪，不重复占用人工复盘名额。")

    print("\n  分级：A(优先级≥75 且通过硬过滤) / B(60-74 且通过硬过滤) / C(观察池或硬过滤未通过)")
    print("  方式：确认追入=已进主升(7/8) | 启动埋伏=刚向上离开中枢(5) | 新鲜=距启动信号天数")
    print(f"  复盘抽样：仅今日新增，最多 {REVIEW_LIMIT} 只；优先覆盖不同行业/启动方式/当前状态")
    ...
    full_df = pd.DataFrame(results)[cols]
    full_df.to_parquet(out, index=False)
    print(f"\n[文件] {out}")
    review_out = OUTPUT_DIR / f"review_{scan_date}.parquet"
    full_df[full_df["复盘精选"]].sort_values("复盘序号").to_parquet(review_out, index=False)
    print(f"[复盘] {review_out}")
...
def _report_experimental(exp_raw, name_map, industry_map, metadata_available):
    ...
    hard_pass = [r for r in exp_raw if not r["过滤原因"]]
    review_pool = actionable if gate else hard_pass
    review_rows = sr.select_diverse_rows(
        review_pool,
        REVIEW_LIMIT,
        priority_key="priority",
        diversity_keys=("行业", "dec_regime"),
        code_key="代码",
    )
    review_rank = {r["代码"]: i for i, r in enumerate(review_rows, 1)}
    for r in exp_raw:
    ...
    sample_kind = "可操作复盘" if gate else "门关结构抽样"
    print(f"  人工复盘：{sample_kind} {len(review_rows)} 只（上限 {REVIEW_LIMIT}）；完整候选继续全部写入前向日志")
    header = (
    ...
    print(f"  [文件] {out}")
    review_out = OUTPUT_DIR / f"review_exp_delay5_{dec_date}.parquet"
    df[df["复盘精选"]].sort_values("复盘序号").to_parquet(review_out, index=False)
    print(f"  [复盘] {review_out}")
